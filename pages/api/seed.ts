...
export default async function handler(
  ...
  const indianCuisineId =
    cuisines.find((cuisine) => cuisine.name === "indian")?.id || 111;
  const mexicanCuisineId =
    cuisines.find((cuisine) => cuisine.name === "mexican")?.id || 222;
  const italianCuisineId =
    cuisines.find((cuisine) => cuisine.name === "italian")?.id || 333;

  const ottawaLocationId =
    locations.find((location) => location.name === "ottawa")?.id || 444;
  const torontoLocationId =
    locations.find((location) => location.name === "toronto")?.id || 556;
  const niagaraLocationId =
    locations.find((location) => location.name === "niagara")?.id || 666;
